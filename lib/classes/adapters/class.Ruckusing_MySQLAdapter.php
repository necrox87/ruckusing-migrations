...
class Ruckusing_MySQLAdapter extends Ruckusing_BaseAdapter implements Ruckusing_iAdapter {
    ...
    public function native_database_types() {
        $types = array(
            ...
            'enum' => array('name' => "enum", 'null' => false)
        );
        return $types;
    }
...
//column_info
    ...
    public function add_index($table_name, $column_name, $options = array()) {
        ...
        $sql = sprintf("CREATE %sINDEX %s ON %s(%s)", $unique ? "UNIQUE " : "", $index_name, $this->identifier($table_name), join(", ", $cols));
        return $this->execute_ddl($sql);
    }
...
//has_index
    ...
    public function type_to_sql($type, $options = array()) {
        ...
        if ($type == "decimal") {
            ...
            if ($values != null) {
                $column_type_sql .= sprintf("(%s)", $values);
            } else {
                throw new Ruckusing_ArgumentException("Error adding enum column: enum values not specified");
            }//precision			
        } {
